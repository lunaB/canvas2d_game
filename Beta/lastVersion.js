...
var interval = Array(20);//온로드에 인터벌을 추가하려면 1높여줍니다.
function onLoad(){ //interval 개수 4개
    ...
    interval[0] = setInterval(function(){
		var randSpeed = Math.random()*2.01+1;
...
denger[0] = { //첫 빔
    ...
    drowDengerTimeE:2,  //경고끝시간
    drowAble:false,     //그리기에서 확인하기위한 변수
    xy:'x',             //어떤축에서 빔이나올지
    drowDengerWH:200,   //경고 높이나 너비
...
function mapDanger(){ //10 interval
    interval[5] = setInterval(function() {
		...
		if(denger[dengerIndex].drowDengerTimeS <= timer && timer < denger[dengerIndex].drowDengerTimeE) { //1,2
			...
			if(locationStop){
				denger[dengerIndex].attackLocation = (denger[dengerIndex].xy == 'x' ? player.x : player.y);
				locationStop = false;
			}
			...
			if(denger[dengerIndex].xy == 'x'){
				if(
                    denger[dengerIndex].attackLocation <= (player.x - player.width/2) &&
                    denger[dengerIndex].attackLocation + denger[dengerIndex].drowDengerWH >= (player.x + player.width/2)
                  ){
                    playerState.hp -= denger[dengerIndex].attackDamage/100;
				}
			}else if(denger[dengerIndex].xy == 'y'){
				if(
                    denger[dengerIndex].attackLocation <= (player.y - player.width/2) &&
                    denger[dengerIndex].attackLocation + denger[dengerIndex].drowDengerWH >= (player.y + player.width/2)
                ){
					playerState.hp -= denger[dengerIndex].attackDamage/100;
				}
			}
...
function drow(){
    ...
    if(denger[dengerIndex].drowAble == true){ // 경고
        ...
        denger[dengerIndex].alpha += (10 / ((denger[dengerIndex].drowDengerTimeE - denger[dengerIndex].drowDengerTimeS) * 1000)) / 2; //투명도 낮춰줌
